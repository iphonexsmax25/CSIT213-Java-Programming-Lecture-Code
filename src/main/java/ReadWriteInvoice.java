...
public class ReadWriteInvoice {
    ...
    public static void writeInvoice(ArrayList<Invoice> data, String filename) {
        try (PrintWriter writer = new PrintWriter(filename)){ // inside round bracket --> everything opened inside the round brackets will be closed at the end of the try
            writer.println("Processed Invoice Data");
            // Save all instances in ArrayList to file
    ...
    public static void readInvoice(String filename) {
        ...
        Scanner reader = null;
        try { // do here?
            reader = new Scanner(new File (filename)); // create a file instance based on the filename --> scanner reads from file instead of keyboard
            while (reader.hasNext()) {
                ...
                try {
                    ...
                    if (hours <= 0 || rate <= 0) {
                        // throw Exception to interrupt this iteration
            ...
            writeInvoice(data, "invoice2_out.txt");
        }
        catch (FileNotFoundException err) {
            System.out.println(err);
        }
